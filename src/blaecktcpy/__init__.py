...
class BlaeckTCPy:
    ...
    def __init__(
        ...
        Args:
            ...
            ip: IP address to bind to (default: 127.0.0.1 = localhost)
        ...
        print(
            f"\033[32mblaecktcpy v{__version__}\033[0m — Listening on \033[36m{ip}:{port}\033[0m"
        )
    ...
    def add_signal(
        ...
        if isinstance(signal_or_name, Signal):
            signal = signal_or_name
        elif isinstance(signal_or_name, str):
            signal = Signal(signal_or_name, datatype, value)
        else:
    ...
    def _build_data_msg(
        ...
        for idx in range(start, end + 1):
            ...
            if only_updated and not signal.updated:
                continue
            payload += idx.to_bytes(2, "little") + signal.to_bytes()
            if only_updated:
